...
void drawTree(node * p, bool condense, int maxH, int prevHeight = 0) {
	...
	if(!condense) {
		if (p == NULL) { // height always return -1 ( wrong current level for that node)
			for(double i = 1; i < pow(2,prevHeight); i++) {
				cout << " ";
			}
			for(double i = 0; i < pow(2,prevHeight); i++) {
				cout << "`";
			}
			cout << endl;
		}
			...
			if(!(p->left == NULL && p->right == NULL)) {
				 drawTree(p->left, condense, maxH, height(p));
				 drawTree(p->right, condense, maxH, height(p));
			}
		}
	}
	//condense
